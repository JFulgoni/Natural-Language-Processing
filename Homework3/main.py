...
if __name__ == '__main__':
    ...
    tA = time.time()
    am, asec = divmod(tA-t0, 60)
    print 'Part A Time: ' + str(am) + ':' + str(asec)

    B.run(train_set, test_set, language, best_answer)
    tB = time.time()
    bm, bs = divmod(tB-tA, 60)
    print 'Part B Time: ' + str(bm) + ':' + str(bs)

    tm, ts = divmod(tB-t0, 60)
    print 'Total Time: ' + str(tm) + ':' + str(ts)